...
for feature in js['features']:
    ...
    for i in range(len(point_data_list)):
        ...
        event_date_month = event_date.split(' ')[0].split('-')[1]
        event_date_year = event_date.split(' ')[0].split('-')[0]
        if polygon.contains(point):
            # print('Found containing polygon:', feature['properties']['dname'])
            if feature['properties']['dname'] in district_point_group:
                district_point_group[feature['properties']['dname']] += [[point_label, event_date_month, event_date_year]]
            else:
...
for i in district_point_group:
    for j in range(len(district_point_group[i])):
        district_accident_month.append(i+str(district_point_group[i][j][1] + str(district_point_group[i][j][2])))
